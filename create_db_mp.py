...
from pharmacophore import Pharmacophore2 as Pharmacophore, read_smarts_feature_file, load_multi_conf_mol


def create_tables(cursor, bin_step, smarts, store_coords):
    cursor.execute("CREATE TABLE conformers(conf_id INTEGER PRIMARY KEY AUTOINCREMENT, mol_name TEXT, "
                   "mol_stereo_id TEXT, ph_hash BLOB, ph_stereo INTEGER)")
...
def insert_res_db(cur, res, store_coords, stereo_id):
    for item in res:
        if stereo_id:
            ...
            mol_name = item[0]
            mol_stereo_id = '0'
        # cur.execute("INSERT OR IGNORE INTO molecules(mol_name) VALUES(?, ?, ?)", (mol_id, mol_name, mol_stereo_id))
        # cur.execute("SELECT rowid FROM molecules WHERE mol_name = '%s'" % mol_name)
        # mol_id = cur.fetchone()[0]
        cur.execute("INSERT INTO conformers VALUES(NULL, ?, ?, ?, ?)", (mol_name, mol_stereo_id, item[1], item[2]))
        if store_coords and item[3] is not None:
            cur.execute("SELECT MAX(conf_id) FROM conformers")
            conf_id = cur.fetchone()[0]
            cur.executemany("INSERT INTO feature_coords VALUES (?, ?, ?, ?, ?)", ((conf_id, i[0], *i[1]) for i in item[3]))
...
def compress_db(cursor, store_coords):
    ...
    cursor.execute("DELETE FROM conformers WHERE rowid NOT IN (SELECT MIN(rowid) "
                   "FROM conformers GROUP BY mol_name, mol_stereo_id, ph_hash, ph_stereo)")
    if store_coords:
...
def insert_res_txt(f, res, lines_set, stereo_id):
    for item in res:
        if stereo_id:
            ...
            mol_name = item[0]
            mol_stereo_id = '0'
        record = (mol_name, mol_stereo_id, item[1], item[2])
        if record not in lines_set:
            f.write("%s\t%s\t%s\t%i\n" % record)
            lines_set.add(record)


def prep_input(fname, id_field_name, smarts, bin_step, store_coords, multiconf, tolerance):
    if fname is None:
    ...
    for mol, mol_name in read_iterator:
        yield mol, mol_name, smarts, bin_step, store_coords, multiconf, tolerance
...
def process_mol(mol, mol_name, smarts, bin_step, store_coords, multiconf, tolerance):
    # process_factory is within process scope only
    if multiconf:
        if 'process_factory' in globals():
            ps = load_multi_conf_mol(mol, smarts, process_factory, bin_step)
        else:
            ps = load_multi_conf_mol(mol, smarts, None, bin_step)
        output = []
        for p in ps:
            coords = p.get_feature_coords() if store_coords else None
            output.append((mol_name, p.get_signature_md5(), p.get_stereo(tol=tolerance), coords))
        return output
    else:
        ...
        coords = p.get_feature_coords() if store_coords else None
        return [(mol_name, p.get_signature_md5(), p.get_stereo(tol=tolerance), coords)]
...
def main_params(conformers_fname, out_fname, dbout_fname, bin_step, rewrite_db, store_coords, id_field_name, stereo_id,
                smarts_features_fname, rdkit_factory, ncpu, tolerance, verbose):
    ...
    if dbout_fname is not None:
        ...
        cur = conn.cursor()
        create_tables(cur, bin_step, smarts, store_coords)

    # open text file
    if out_fname is not None:
        ftxt = open(out_fname, 'wt')
        ftxt.write("mol_id\tstereo_id\thash\tstereo\n")
    ...
    try:
        for i, res in enumerate(p.imap_unordered(map_process_mol,
                                                 prep_input(conformers_fname, id_field_name, smarts, bin_step, store_coords, multiconf, tolerance),
                                                 chunksize=10), 1):
            counter += len(res)
            if dbout_fname is not None:
...
def main():
    ...
    parser.add_argument('-b', '--bin_step', default=2,
                        help='binning step. Default: 2.')
